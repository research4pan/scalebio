...
class Minmax():
    ...
    def run(
        ...
        def train_loss(model, samp_prob_p, loss_scale_factor, id_str='', minibatch=None, bin_counts=None):
            return loss_on_sampler(
                model=model,
                samp_prob_p=samp_prob_p,
                accelerator=accelerator,
        ...
        #             args=config.args,
        #         )
        def val_loss(model, loss_scale_factor, id_str='', minibatch=None):
            return loss_on_sampler(
                model=model,
                samp_prob_p=None,
                accelerator=accelerator,
        ...
        model_u, optim_u, model_w, optim_w, train_loader, val_loader_for_train, val_loader_for_eval, test_loader = accelerator.prepare(model_u, optim_u, model_w, optim_w, train_loader, val_loader_for_train, val_loader_for_eval, test_loader)
        ...
        for i in range(num_outer_iter):
            ...
            for k in range(num_inner_iter):
                ...
                if self.config.args.lisa and global_step % self.config.args.lisa_step == 0:
                    self.lisa(model_u, n_layers=self.config.args.lisa_n_layers)
                    self.lisa(model_w, n_layers=self.config.args.lisa_n_layers)
                ...
                for accu_step in range(self.config.gradient_accumulation_steps):
                    try:
                        batch = next(self.val_loader_for_train_iter)
                    except StopIteration as e:
                        self.val_loader_for_train_iter = iter(val_loader_for_train)
                        batch = next(self.val_loader_for_train_iter)
                    val_batch_cache.append(batch)
                    total_non_zero_tokens += torch.count_nonzero(batch['labels'][..., 1:] != -100)
                total_non_zero_tokens = total_non_zero_tokens.to(accelerator.device)
                total_non_zero_tokens = accelerator.gather(total_non_zero_tokens).sum().item()
                # print(f'total_non_zero_tokens-train: {total_non_zero_tokens}')
                for accu_step in range(self.config.gradient_accumulation_steps):
                    self.train_loader_iter = iter(train_loader)
                    try:
                        ...
                        self.train_loader_iter = iter(train_loader)
                        batch = next(self.train_loader_iter)
                    train_batch_cache.append(batch)
                    p_batch_list.append(batch['source'])
                
                bin_count = torch.bincount(torch.cat(p_batch_list), minlength=self.config.num_partitions)
                bin_count = bin_count.to(accelerator.device)
                bin_counts = accelerator.gather(bin_count).reshape(-1, self.config.num_partitions).sum(0)

                if global_step >= self.config.args.warmup_on_w_lambda_steps:
                    for accu_step in range(self.config.gradient_accumulation_steps):
                        # st=time.time()
                        loss_u_from_L2_acc, p_L_u_L2_acc = train_loss(
                            model_u,
                            samp_prob_p,
                            loss_scale_factor=-alpha,
                            # loss_scale_factor=-alpha / self.config.gradient_accumulation_steps,
                            id_str='L2(u, lambd)',
                            minibatch=train_batch_cache[accu_step],
                            bin_counts=bin_counts,
                        )
                        loss_u_from_L2+=accelerator.gather(loss_u_from_L2_acc).detach().cpu().sum()
                        p_L_u_L2+=accelerator.gather(p_L_u_L2_acc).detach()
                    # accelerator.clip_grad_norm_(model_u.parameters(), 1.0)
                    if self.config.args.clip_model:
                        u_norm = model_u.clip_grad_norm_(1.0 * alpha, norm_type=2.0)
                    else:
                        u_norm = model_u.clip_grad_norm_(1e9, norm_type=2.0)
                    optim_u.step()
                    optim_u.zero_grad()
                ...
                for accu_step in range(self.config.gradient_accumulation_steps):
                    loss_w_from_L2_acc, p_L_w_L2_acc = train_loss(
                        model_w,
                        samp_prob_p,
                        # reuse_last_samples=(global_step >= self.config.args.warmup_on_w_lambda_steps),    # Reuses data samples in L2(u, lambd)
                        loss_scale_factor=alpha,
                        # loss_scale_factor=alpha / self.config.gradient_accumulation_steps,
                        id_str='L2(w, lambd)',
                        minibatch=train_batch_cache[accu_step],
                    ...
                    loss_w_from_L2+=accelerator.gather(loss_w_from_L2_acc).detach().cpu().sum()
                    # print(p_L_w_L2_acc, p_L_w_L2_acc.device)
                ...
                for accu_step in range(self.config.gradient_accumulation_steps):
                    loss_w_from_L1_acc, _ = val_loss(
                        ...
                        loss_scale_factor=1.0 / total_non_zero_tokens,
                        id_str='L1(w, lambd)',
                        minibatch=val_batch_cache[accu_step],
                    )
                    loss_w_from_L1+=accelerator.gather(loss_w_from_L1_acc).detach().cpu().sum()

                    # tol=time.time()-st
                    # profiler.step()
                # accelerator.clip_grad_norm_(model_w.parameters(), 1.0)
                if self.config.args.clip_model:
                    w_norm = model_w.clip_grad_norm_(1.0 * alpha, norm_type=2.0)
                else:
                    w_norm = model_w.clip_grad_norm_(1e9, norm_type=2.0)
                optim_w.step()
                optim_w.zero_grad()
                ...
                if self.config.args.lambda_grad_clip_value is not None:
                    torch.nn.utils.clip_grad_value_(samp_prob_p.parameters(), self.config.args.lambda_grad_clip_value)
                    assert torch.max(torch.abs(samp_prob_p.p.grad)) <= self.config.args.lambda_grad_clip_value, 'clipping grad failed'
                
                if self.config.args.lambda_grad_clip_norm is not None:
                    torch.nn.utils.clip_grad_norm_(samp_prob_p.parameters(), self.config.args.lambda_grad_clip_norm)
                    assert torch.linalg.vector_norm(samp_prob_p.p.grad) <= self.config.args.lambda_grad_clip_norm, 'clipping grad failed'
                ...
                stat_dict = {
                        ...
                        # 'L2(w, lambda) no alpha': loss_w_from_L2,
                        'L2(w, lambda)': loss_w_from_L2 / alpha,
                        'L2(u, lambda)': loss_u_from_L2 / -alpha,
                        'L2(w, lambda) - L2(u, lambda)': (loss_w_from_L2 + loss_u_from_L2) / alpha,
                        'grad u norm div alpha': u_norm / alpha,
                        'grad w norm div alpha': w_norm / alpha,
                        'step': global_step,
                        'time': time.time() - start_time,
                        ...
                        'lr': lr,
                        # 'p_L_u_L2': p_L_u_L2.detach().cpu(),
                        # 'p_L_w_L2': p_L_w_L2.detach().cpu(),
                    }
                logging_stat_dict(
                    stat_dict,
