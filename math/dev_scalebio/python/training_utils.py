...
def loss_on_batch(
    ...
    if samp_prob_p is not None:
        ...
        def loss_per_sample(logits, labels):
            ...
            loss = torch.where(count_per_exp != 0, loss.sum(dim=-1) / count_per_exp, 0.)

            return loss
    
        crossent_loss=loss_per_sample(predicted_logits, y_batch)
        loss=samp_prob_p(crossent_loss, p_batch, bin_counts)
        w_nj=1.0/bin_counts[p_batch]
        crossent_loss=torch.mul(w_nj, crossent_loss)

        # with FullyShardedDataParallel.summon_full_params(samp_prob_p, with_grads=True):
        per_partition_loss=torch.zeros_like(samp_prob_p.prob).cuda()
        per_partition_loss.index_add_(0, p_batch.cuda(), crossent_loss)
    else:
        n_items = torch.stack([
                    torch.count_nonzero(y_batch[..., 1:] != -100)
                ]).sum()
        loss = model(x_batch, labels=y_batch, attention_mask=attn_mask, return_dict=True).loss
        loss = loss * n_items

    loss = loss_scale_factor * loss * accelerator.num_processes

    accelerator.backward(loss)

    loss = loss / accelerator.num_processes

    per_partition_loss = loss_scale_factor * per_partition_loss * accelerator.num_processes
...
def loss_on_sampler(
    ...
    x_batch=minibatch['input_ids']
    y_batch=minibatch['labels']
    p_batch=minibatch['source']
    attn_mask=minibatch['attention_mask']
    ...
    # print(f'batch_loss: {batch_loss}, id_str: {id_str}')
    sum_loss += batch_loss
...
def evaluate(model, accelerator, data_loader, args):
    ...
    for batch in data_loader:
        ...
        total_non_pad_tokens += torch.count_nonzero(batch['labels'] != -100)
    ...
    total_non_pad_tokens = accelerator.gather(total_non_pad_tokens).sum().item()
    # print(f'total_non_pad_tokens-eval: {total_non_pad_tokens}')

    sum_loss = accelerator.gather(sum_loss).detach().cpu().sum() / total_non_pad_tokens
...
def get_cosine_schedule_with_warmup(lr_scheduler_args_dict):
    if not 'min_lr' in lr_scheduler_args_dict:
        raise ValueError(
            f'min_lr must be provided, e.g. "min_lr=0.0"'
        )
    import math
    min_lr = lr_scheduler_args_dict['min_lr']
    warmup_ratio = lr_scheduler_args_dict.get('warmup_ratio', 0.03)

    def cosine_schedule_with_warmup(init_lr, current_step, num_steps):
        warmup_steps = math.ceil(int(num_steps * warmup_ratio))
        if init_lr < min_lr:
            raise ValueError(
                f'initial lr "{init_lr}"'
                f' must be no less than "{min_lr}"'
        ...
        if current_step < warmup_steps:
            ...
            lr_factor = 0.5 * (1 + np.cos(np.pi * (current_step - warmup_steps) / (num_steps - warmup_steps)))
            lr = (init_lr - min_lr) * lr_factor + min_lr
        return lr
...
def get_lr_scheduler(lr_scheduler_args_dict):
    ...
    if name == 'cosine_w_warmup':
        return get_cosine_schedule_with_warmup(lr_scheduler_args_dict)
