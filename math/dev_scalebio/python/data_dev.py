        ...
        if transform:
            ...
            if chunk_long_text:
                for i in tqdm(range(len(self.data)), miniters=500):
                    chunked.extend([item for sublist in map(transform, self.data) for item in sublist])
                self.data=chunked
            else:
                for i in tqdm(range(len(self.data)), miniters=500):
...
def tokenize_text_only(data_point: Dict = None,
    ...
    for i in range(0, len(tokens), max_length):
        res.append({"input_ids": copy.deepcopy(tokens[i : i + max_length]), "labels": copy.deepcopy(tokens[i : i + max_length]), "source": data_point["source"]})
    print(len(res))
    return res
