...
def prepare_data(args):
    ...
    #ds = load_dataset("PMATHWORK/prm_math_prompt", split='train')
...
def main(args):
    ...
    if "gemma" in args.model_name_or_path:
        ...
        stop_tokens = ["<|im_end|>", "<|im_start|>user", "```output"]
    else:
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        #sample = {'parrent_idx': example['parrent_idx'], "idx": idx, "gt": example['gt'], "prompt": example['my_solu'], 'traj': example['traj']}        #add remain fields
        for key in [
            "level",
            "type",
            "unit",
